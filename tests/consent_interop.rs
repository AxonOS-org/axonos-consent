...
mod json_tests {
    ...
    #[test] fn state_transitions() {
        ...
        for tv in root["vectors"].as_array().unwrap() {
            let id = tv["id"].as_str().unwrap_or("?");
            let (b, a) = match (tv.get("state_before").and_then(|v|v.as_str()), tv.get("state_after").and_then(|v|v.as_str())) {
...
#[test] fn eng_register() { let mut e = ConsentEngine::new(); e.register_peer([1;16], 0).unwrap(); assert_eq!(e.get_state(&[1;16]), Some(ConsentState::Granted)); }
...
fn wf() -> ConsentFrame { ConsentFrame::Withdraw(ConsentWithdraw {
    scope: Scope::Peer, reason_code: None, reason: None, epoch: None, timestamp_ms: Some(1), timestamp_us: None,
}) }
fn sf() -> ConsentFrame { ConsentFrame::Suspend(ConsentSuspend {
    reason_code: None, reason: None, timestamp_ms: Some(1), timestamp_us: None,
}) }
fn rf() -> ConsentFrame { ConsentFrame::Resume(ConsentResume { timestamp_ms: Some(1), timestamp_us: None }) }

// Row 1: GRANTED
#[test] fn af_granted_withdraw() { assert_eq!(ConsentState::Granted.apply_frame(&wf()), Ok(ConsentState::Withdrawn)); }
#[test] fn af_granted_suspend()  { assert_eq!(ConsentState::Granted.apply_frame(&sf()), Ok(ConsentState::Suspended)); }
#[test] fn af_granted_resume()   { assert_eq!(ConsentState::Granted.apply_frame(&rf()), Ok(ConsentState::Granted)); }

// Row 2: SUSPENDED
#[test] fn af_suspended_withdraw() { assert_eq!(ConsentState::Suspended.apply_frame(&wf()), Ok(ConsentState::Withdrawn)); }
#[test] fn af_suspended_suspend()  { assert_eq!(ConsentState::Suspended.apply_frame(&sf()), Ok(ConsentState::Suspended)); }
#[test] fn af_suspended_resume()   { assert_eq!(ConsentState::Suspended.apply_frame(&rf()), Ok(ConsentState::Granted)); }

// Row 3: WITHDRAWN — all rejected
#[test] fn af_withdrawn_withdraw() { assert_eq!(ConsentState::Withdrawn.apply_frame(&wf()), Err(TransitionError::AlreadyWithdrawn)); }
#[test] fn af_withdrawn_suspend()  { assert_eq!(ConsentState::Withdrawn.apply_frame(&sf()), Err(TransitionError::AlreadyWithdrawn)); }
#[test] fn af_withdrawn_resume()   { assert_eq!(ConsentState::Withdrawn.apply_frame(&rf()), Err(TransitionError::AlreadyWithdrawn)); }
