...
pub fn check_frame(frame: &ConsentFrame) -> InvariantResult {
    ...
    match frame {
        ConsentFrame::Withdraw(w) => {
            ...
            if let Some(0) = w.timestamp_ms { r.add_violation(InvariantViolation::ZeroTimestampMs); }
            if let Some(0) = w.timestamp_us { r.add_violation(InvariantViolation::ZeroTimestampUs); }

            // MUST: reason length bounded
            if let Some(ref reason) = w.reason {
        ...
        ConsentFrame::Suspend(s) => {
            if let Some(0) = s.timestamp_ms { r.add_violation(InvariantViolation::ZeroTimestampMs); }
            if let Some(0) = s.timestamp_us { r.add_violation(InvariantViolation::ZeroTimestampUs); }
        ...
        ConsentFrame::Resume(re) => {
            if let Some(0) = re.timestamp_ms { r.add_violation(InvariantViolation::ZeroTimestampMs); }
            if let Some(0) = re.timestamp_us { r.add_violation(InvariantViolation::ZeroTimestampUs); }
